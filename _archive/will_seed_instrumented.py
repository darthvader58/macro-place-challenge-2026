"""Will's seed placer with per-stage timing instrumentation. Audit-only.

Wraps the original WillSeedPlacer with time.perf_counter() at each
major boundary inside place(). Otherwise identical to
submissions/will_seed/placer.py.
"""

import sys, io, math, random, time
import torch
import numpy as np
